...
export class MoviVideoDecoder {
  ...
  private pendingChunks: Array<{data: Uint8Array, timestamp: number, keyframe: boolean}> = [];
  // ... (fields same) ...
   ...
   */
  async configure(track: VideoTrack, extradata?: Uint8Array): Promise<boolean> {
    ...
    if (!codecString) {
         Logger.debug(TAG, 'CodecParser returned null, falling back to manual mapping');
         codecString = this.mapCodecToWebCodecs(track.codec, track.width, track.height, track.profile, track.level);
    }
    ...
    if (track.colorPrimaries || track.colorTransfer || track.colorSpace) {
      ...
      Logger.info(TAG, `Decoder color space: primaries=${track.colorPrimaries}, transfer=${track.colorTransfer}, matrix=${track.colorSpace}`);
    }
    ...
    if (description && description.length > 4) {
        const isAnnexB = (description[0] === 0 && description[1] === 0 && description[2] === 1) || 
                         (description[0] === 0 && description[1] === 0 && description[2] === 0 && description[3] === 1);
        
        if (isAnnexB) {
            Logger.warn(TAG, 'Extradata appears to be Annex B (NAL units). WebCodecs hvc1/avc1 requires MP4 box format (hvcC/avcC). Stripping extradata from config to avoid initialization error.');
    ...
    try {
      ...
      if (!support.supported && config.colorSpace) {
         Logger.info(TAG, `Codec config failed with color space. Retrying without explicit color metadata.`);
         const configNoColor = { ...config };
         delete configNoColor.colorSpace;
         
         const supportNoColor = await VideoDecoder.isConfigSupported(configNoColor);
         if (supportNoColor.supported) {
             Logger.info(TAG, `Codec supported WITHOUT explicit color metadata. Using stripped config.`);
      ...
      if (!support.supported) {
        Logger.warn(TAG, `Codec config not supported: ${config.codec}`);
         ...
         if (codecString && codecString.startsWith('hvc1.4')) {
             ...
             if (codecString !== rextFallback) {
                 Logger.info(TAG, `HEVC Rext detected. Retrying with compatible fallback string: ${rextFallback}`);
                 ...
                 if (rextSupport.supported) {
                      Logger.info(TAG, `HEVC Rext fallback string IS supported. Switching to ${rextFallback}`);
         ...
         const manualCodec = this.mapCodecToWebCodecs(track.codec, track.width, track.height, track.profile, track.level);
         ...
         if (manualCodec && manualCodec !== codecString) {
             ...
             const manualSupport = await VideoDecoder.isConfigSupported(manualConfig);
             
             if (manualSupport.supported) {
                 Logger.info(TAG, `Manual codec string IS supported. Using ${manualCodec} instead of ${codecString}`);
        ...
        if (!support.supported) {
            Logger.warn(TAG, `Codec not supported by hardware: ${codecString}. Trying software.`);
      ...
      if (config.colorSpace) {
          try {
              Logger.info(TAG, 'Retrying config check without color space after error');
              delete config.colorSpace;
              const support = await VideoDecoder.isConfigSupported(config);
              if (support.supported) {
                  Logger.info(TAG, 'Codec check passed after removing color space. Proceeding.');
    ...
    try {
      ...
      Logger.info(TAG, `Configured: ${codecString} ${track.width}x${track.height}`);
      return true;
    } catch (error) {
  ...
  private async initSoftwareDecoder(): Promise<boolean> {
      ...
      if (!this.bindings) {
          Logger.error(TAG, 'Cannot switch to software decoder: bindings not available');
          return false;
      }
      ...
      if (success) {
          ...
          if (this.pendingChunks.length > 0) {
              Logger.info(TAG, `Processing ${this.pendingChunks.length} pending chunks for software decoder`);
  ...
  private lastChunkInfo: { timestamp: number, keyframe: boolean, size: number } | null = null;
  ...
  decode(data: Uint8Array, timestamp: number, keyframe: boolean): void {
    ...
    if (this.useSoftware && this.swDecoder) {
        ...
        if (keyframe && this.shouldRetryHardware(data)) {
             Logger.info(TAG, `Found a sync frame! Attempting hardware resurrection (Attempt ${this.hardwareRetryCount + 1})...`);
    ...
    try {
      this.decoder.decode(chunk);
       ...
       if (this.useSoftware) {
           Logger.warn(TAG, 'Hardware decode failed, queuing chunk for software decoder');
           this.pendingChunks.push({data, timestamp, keyframe});
       }
    }
  }
  ...
  private recoverFromError(error: Error) {
    const isKeyFrameError = error.message && (
    ...
    if (isKeyFrameError) {
         ...
         if (this.isResurrecting) {
              Logger.warn(TAG, "Hardware resurrection failed on sync frame. Returning to software decoder.");
              this.isResurrecting = false;
         ...
         Logger.warn(TAG, `Decoding warning: Frame was marked as keyframe but decoder rejected it (Open GOP?). Timestamp: ${this.lastChunkInfo?.timestamp}. Count (OpenGOP): ${this.openGopErrorCount}`);
         
         // If we keep hitting these, hardware decoder is too strict. Fallback to software.
         if (this.openGopErrorCount > 15) {
              Logger.error(TAG, 'Persistent Open GOP errors detected. Switching to software decoder.');
              this.initSoftwareDecoder();
              return;
         }
         ...
         if (this.decoder && this.decoder.state !== 'closed') {
           try {
             ...
             Logger.warn(TAG, 'Fast reset failed during Open GOP recovery, proceeding to full recreation');
           }
         }
    } else {
         Logger.error(TAG, `Decoding error details: ${JSON.stringify(errorInfo)}. Count: ${this.errorCount}`);
    }

    if (this.errorCount >= MoviVideoDecoder.MAX_ERRORS) {
      Logger.error(TAG, `Max errors (${MoviVideoDecoder.MAX_ERRORS}) exceeded within short duration. Emitting fatal error.`);
    ...
    if (this.currentProfile === 4 && this.lastConfig?.codec.startsWith('hvc1.4.')) {
      ...
      if (this.lastConfig.codec !== fallbackStr) {
          Logger.info(TAG, `Attempting recovery by switching to compatible fallback string: ${fallbackStr}`);
          ...
          if (this.lastConfig.description && this.lastConfig.description.length > 5) {
              ...
              if (originalProfile === 4) { // Only patch if it is Rext
                   patched[1] = (patched[1] & 0xE0) | 2; 
                   Logger.warn(TAG, `Patched HEVC extradata: Spoofed Profile IDC from ${originalProfile} to 2 (Main 10) to bypass strict hardware checks.`);
          ...
          try {
            ...
            Logger.warn(TAG, 'HEVC Rext fallback string also failed. Attempting reset/recreation.');
      }
    }
   ...
   */
  private mapCodecToWebCodecs(codec: string, _width: number, _height: number, profile?: number, _level?: number): string | null {
    ...
    // H.265 / HEVC
    if (codecLower === 'hevc' || codecLower === 'h265' || codecLower === 'hvc1') {
      // Handle HEVC profiles
      if (profile === 4) { // FF_PROFILE_HEVC_REXT
        Logger.info(TAG, 'Detected HEVC Rext profile. Trying compatible Main10 string map.');
      ...
      if (profile === 2) {
          ...
          Logger.info(TAG, `Mapping HEVC Main 10 profile (2) to hvc1.2.4.${levelStr}.B0`);
    ...
    if (codecLower === 'vp9') {
      ...
      if (profile === 2) {
          ...
          Logger.info(TAG, 'Mapping VP9 Profile 2 to vp09.02.51.10.01.09.16.09.00 (HDR)');
  ...
  private isLikelySyncFrame(data: Uint8Array): boolean {
    ...
    try {
        ...
        else if (data[0] === 0 && data[1] === 0 && data[2] === 0 && data[3] === 1) headerPos = 4;
        ...
        // H.265 (HEVC)
        if (codec.includes('hvc1') || codec.includes('hev1') || codec.includes('h265')) {
