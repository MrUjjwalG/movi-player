...
export class MoviAudioDecoder {
  ...
  async configure(track: AudioTrack, extradata?: Uint8Array): Promise<boolean> {
    ...
    if (track.channels > 2) {
      Logger.info(TAG, `Forcing software decoding for multi-channel audio: ${track.channels} channels`);
      return this.initSoftwareDecoder();
    }
    ...
    if (!codecString) {
      Logger.warn(TAG, `Codec ${track.codec} not natively supported, trying software.`);
      return this.initSoftwareDecoder();
    }
    ...
    this.decoder = new AudioDecoder({
      ...
      error: async (error) => {
        ...
        if (!this.useSoftware && !this.hasTriedSoftwareFallback && this.currentTrack) {
          Logger.warn(TAG, 'Hardware decoder error detected, automatically switching to software decoder');
          this.hasTriedSoftwareFallback = true;
          
          // Try to switch to software decoder
          const switched = await this.initSoftwareDecoder();
          if (switched) {
            Logger.info(TAG, 'Successfully switched to software decoder after hardware error');
            // Don't call onError if we successfully switched - the error is handled
            return;
          } else {
            Logger.error(TAG, 'Failed to switch to software decoder, error will be propagated');
          }
        }
    ...
    try {
      ...
      Logger.info(TAG, `Configured: ${codecString} ${track.sampleRate}Hz ${track.channels}ch`);
      return true;
    } catch (error) {
  ...
  private needsSoftwareDecoding(codec: string): boolean {
    const transcodingCodecs = ['eac3', 'ac3', 'dts', 'dca', 'truehd', 'mlp', 'opus'];
  ...
  decode(data: Uint8Array, timestamp: number, keyframe: boolean): void {
    ...
    try {
      ...
      // Automatically fallback to software decoder if not already using it
      if (!this.useSoftware && !this.hasTriedSoftwareFallback && this.currentTrack) {
        Logger.warn(TAG, 'Decode exception detected, automatically switching to software decoder');
        this.hasTriedSoftwareFallback = true;
        ...
        // Try to switch to software decoder
        this.initSoftwareDecoder().then((switched) => {
          if (switched) {
            Logger.info(TAG, 'Successfully switched to software decoder after decode exception');
